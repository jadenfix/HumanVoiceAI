    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for emotion_name, emotion_id in emotions.items():
            ...
            try:
                ...
                assert output_path.exists(), f"Output file not created for {emotion_name}"
                assert output_path.stat().st_size > 0, f"Empty output file for {emotion_name}"
                
                print(f"  ✓ Successfully generated {output_path.name} ({output_path.stat().st_size} bytes)")

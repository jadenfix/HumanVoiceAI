...
class Generator(nn.Module):
    ...
    def __init__(self, 
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            ...
            self.noise_convs.append(nn.utils.weight_norm(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.num_upsamples):
            ...
            # Add noise
            noise = torch.randn(x.size(0), 1, x.size(2) * self.ups[i][1].stride[0]).to(x.device)
...
class DiscriminatorP(nn.Module):
    ...
    def __init__(self, period: int, kernel_size: int = 5, stride: int = 3):
        ...
        # Layers
        self.convs = nn.ModuleList([
            nn.utils.weight_norm(nn.Conv2d(1, 32, (kernel_size, 1), (stride, 1), padding=(kernel_size // 2, 0))),
            nn.utils.weight_norm(nn.Conv2d(32, 128, (kernel_size, 1), (stride, 1), padding=(kernel_size // 2, 0))),
            nn.utils.weight_norm(nn.Conv2d(128, 512, (kernel_size, 1), (stride, 1), padding=(kernel_size // 2, 0))),
            nn.utils.weight_norm(nn.Conv2d(512, 1024, (kernel_size, 1), (stride, 1), padding=(kernel_size // 2, 0))),
            nn.utils.weight_norm(nn.Conv2d(1024, 1024, (kernel_size, 1), 1, padding=(kernel_size // 2, 0))),
        ])

...
class SERTrainer(BaseTrainer):
    ...
    def _compute_class_weights(self) -> Optional[torch.Tensor]:
        ...
        """
        if not hasattr(self.config.training, 'use_class_weights') or not self.config.training.use_class_weights:
            return None
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(data_loader, desc=f"Evaluating {split}")):
                # Move data to device
                features = batch['features'].to(self.device).unsqueeze(1)  # Add channel dim
                labels = batch['labels'].to(self.device)

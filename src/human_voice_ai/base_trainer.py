...
class BaseTrainer:
    ...
    def _train_epoch(self) -> Dict[str, float]:
        ...
        with tqdm(self.train_loader, desc=f"Epoch {self.epoch+1}", unit="batch") as t:
            for batch_idx, batch in enumerate(t):
                ...
                t.set_postfix(
                    {
                        "loss": step_metrics.get("loss", 0).item()
                        if hasattr(step_metrics.get("loss", 0), "item")
                        else step_metrics.get("loss", 0),
                        "acc": step_metrics.get("accuracy", 0).item()
                        if hasattr(step_metrics.get("accuracy", 0), "item")
                        else step_metrics.get("accuracy", 0),
                    }
                )
    ...
    def test(self) -> Dict[str, float]:
        ...
        test_metrics = {"test_loss": 0.0, "test_accuracy": 0.0}
    ...
    def _log_metrics(
        self, split: str, metrics: Dict[str, float], global_step: Optional[int] = None
    ) -> None:
        """
    ...
    def load_checkpoint(self, checkpoint_path: Union[str, Path]) -> None:
        ...
        print(
            f"Loaded checkpoint from {checkpoint_path} (epoch {self.epoch}, global step {self.global_step})"
        )

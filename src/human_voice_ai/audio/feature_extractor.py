...
class FeatureExtractor(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('window', torch.hann_window(self.win_length, device=self._device))
    ...
    def extract_from_waveform(self, waveform: torch.Tensor, sample_rate: int) -> torch.Tensor:
        ...
        if sample_rate != self.sample_rate:
            if sample_rate <= 0 or self.sample_rate <= 0:
                raise ValueError(f"Invalid sample rates: input={sample_rate}, model={self.sample_rate}")
        ...
        # Combine features
        try:
            features = torch.cat([mel_spec, f0, energy], dim=1)  # [min_time_steps, n_mels + 2]
    ...
    def _extract_mel_spectrogram(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        if actual_frames != expected_frames:
            print(f"  Warning: Frame mismatch! Expected {expected_frames}, got {actual_frames}")
    ...
    def _extract_pitch(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        actual_frames = (waveform_padded.size(-1) - self.win_length) // self.hop_length + 1
        print(f"Pitch - Padded length: {waveform_padded.size(-1)}, Actual frames: {actual_frames}")
        if actual_frames != expected_frames:
            print(f"  Warning: Frame mismatch! Expected {expected_frames}, got {actual_frames}")
        ...
        fft_frames = torch.fft.rfft(frames, dim=-1, n=self.n_fft*2)  # [channels, n_frames, n_fft+1]
        power_spectrum = fft_frames.real.pow(2) + fft_frames.imag.pow(2)
        autocorr = torch.fft.irfft(power_spectrum, dim=-1, n=self.n_fft*2)  # [channels, n_frames, n_fft*2]
        ...
        autocorr = autocorr[..., min_period:max_period+1]  # [channels, n_frames, max_period-min_period+1]
        
        # Find peaks
        max_vals, max_idxs = torch.max(autocorr, dim=-1)  # [channels, n_frames]
        
        # Convert to frequency
        pitch = self.sample_rate / (max_idxs.float() + min_period + 1e-10)  # [channels, n_frames]
    ...
    def _extract_energy(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        expected_frames = ((seq_len + 2*pad - self.win_length) // self.hop_length) + 1
        print(f"Energy - Input length: {seq_len}, Padded length: {waveform.size(-1)}, Expected frames: {expected_frames}")
            
        assert waveform.dim() == 2, f"Expected 2D input, got {waveform.dim()}D"
        
        # Pad the waveform to ensure complete frames
        pad_len = (self.hop_length - (waveform.size(-1) % self.hop_length)) % self.hop_length
        if pad_len > 0:

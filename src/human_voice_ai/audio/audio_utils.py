...
def load_audio(
    ...
    if orig_sample_rate != sample_rate:
        resampler = T.Resample(
            orig_freq=orig_sample_rate, new_freq=sample_rate, dtype=waveform.dtype
        ).to(device)
...
def save_audio(
    ...
    # Save the file
    torchaudio.save(filepath, waveform, sample_rate, bits_per_sample=bits_per_sample)
...
def normalize_audio(
    waveform: torch.Tensor, target_level_db: float = -16.0, max_gain_db: float = 30.0
) -> torch.Tensor:
    """
    ...
    gain = min(max_gain_db, target_level_db - current_level_db)
...
def time_stretch(
    ...
    phase_advance = torch.linspace(
        0, torch.pi * hop_length, stft.shape[-2], device=stft.device
    )
    ...
    for i in range(waveform.size(0)):
        ...
        delta_phase = delta_phase - 2 * torch.pi * torch.round(
            delta_phase / (2 * torch.pi)
        )
        ...
        # Compute new phase
        phase = torch.cat(
            [phase[..., :1], phase[..., 1:] + delta_phase.cumsum(-1)], dim=-1
        )

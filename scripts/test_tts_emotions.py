...
def plot_spectrogram(specgram, title=None, ylabel="Frequency (bin)", ax=None):
    ...
    # Plot
    img = ax.imshow(specgram_db.cpu().numpy(), 
                   cmap='viridis', 
                   origin='lower',
                   aspect='auto',
...
def play_audio(waveform, sample_rate):
    ...
    print(f"Playing audio (sample rate: {sample_rate}Hz, duration: {len(audio[0])/sample_rate:.2f}s)")
...
def main():
    ...
    # Test text
    test_text = "Hello, this is a test of the emotion-controlled text-to-speech system. " \
    ...
    for emotion_name, emotion_id in tqdm(emotions.items(), desc="Generating emotion samples"):
        try:
            # Generate speech
    ...
    while True:
        ...
        try:
            ...
            if 0 <= idx < len(emotions):
                ...
                play_audio(
                    results[emotion_name]['audio'],
                    results[emotion_name]['sample_rate']
                )
            else:

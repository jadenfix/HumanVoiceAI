...
class VocoderDataset(Dataset):
    ...
    def __init__(
        self, data_dir, split="train", sample_rate=22050, segment_length=16384
    ):
...
def train_epoch(
    ...
    for i, batch in enumerate(progress_bar):
        ...
        mel_loss = F.l1_loss(
            mel,
            generator.feature_extractor._extract_mel_spectrogram(y_g_hat.squeeze(1)),
        )
        ...
        if writer is not None:
            ...
            writer.add_scalar(
                "Loss/feature_matching", (loss_fm_f + loss_fm_s).item(), global_step
            )
...
def validate(generator, mpd, msd, val_loader, device, epoch, writer):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(progress_bar):
            ...
            mel_loss = F.l1_loss(
                mel,
                generator.feature_extractor._extract_mel_spectrogram(
                    y_g_hat.squeeze(1)
                ),
            )
...
def save_checkpoint(
    generator, mpd, msd, g_optimizer, d_optimizer, epoch, checkpoint_dir, is_best=False
):
...
def main():
    ...
    parser.add_argument(
        "--data_dir", type=str, default="data/vocoder", help="Path to dataset directory"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="checkpoints/vocoder",
        help="Directory to save checkpoints",
    )
    parser.add_argument(
        "--log_dir", type=str, default="logs/vocoder", help="Directory to save logs"
    )
    parser.add_argument(
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        val_d_loss, val_g_loss, val_mel_loss = validate(
            generator, mpd, msd, val_loader, device, epoch, writer
        )
...
class MultiScaleDiscriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.discriminators = nn.ModuleList(
            [DiscriminatorP(1), DiscriminatorP(2), DiscriminatorP(4)]
        )
